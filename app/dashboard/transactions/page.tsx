              ...
              <TableBody>
                {transactions.map(x => (
                  <TableRow key={x.id} className='text-center'>
                    <TableCell>{format(x.transactionDate, 'MMM do')}</TableCell>
                    <TableCell className='capitalize'>{x.description}</TableCell>
                    <TableCell>
                      <Badge
                        className={cn(x.categoryType === 'Income' ? 'bg-green-400 p-2' : 'bg-red-400 p-2',
                          'text-[14px] px-2 py-1')}
                      >
                        {x.categoryType}
                      </Badge>
                    ...
                    <TableCell>
                      <div className='w-1/2 mx-auto text-right'>${numeral(x.amount).format('0,0.00')}</div>
                    </TableCell>
